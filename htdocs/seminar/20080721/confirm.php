...
?>
  ...
  include ("setup.php");
	...
	foreach ($p as $pnum => $prog) {
		echo '<li>' . $prog["TIME"] . '&nbsp;';
		if (isset($_POST[("p".$pnum)])) {
			$chkp = $_POST[("p".$pnum)];
			echo '<input type="hidden" name="p'.$pnum.'" value="' . htmlspecialchars($chkp) . '" />' . $prog["BODY"][$chkp][0];
		} else {
			echo '<strong>未選択</strong>';
		}
		echo '</li>'. "\n";
	}
	echo '</ul>'. "\n";
...
<?php
	foreach ($p as $pnum => $prog) {
		if (isset($_POST[("p".$pnum)])) {
		echo '<input type="hidden" name="p' . $pnum . '" value="' . htmlspecialchars($_POST[("p".$pnum)]) . '" />' . "\n";
		}
	}
